...
class DesktopOverlayApp:
    def __init__(self, *, app_url: str) -> None:
        ...
        self._launcher_image = _load_photo_image(tk, "app-icon-64.png")
        if self._launcher_image is not None:
            self.canvas.create_image(32, 32, image=self._launcher_image, tags=("launcher",))
        else:
        ...
        self.caption.configure(bg="#0b1220")
    ...
    def start(self) -> None:
        ...
        if not readiness.ready:
            self._active = False
            self.canvas.itemconfigure(self.bubble, fill="#c2410c", outline="#9a3412")
            ...
            self.translation_label.configure(text="未启动真实网页音频翻译")
            return

        self._active = True
        self.canvas.itemconfigure(self.bubble, fill="#16a36f", outline="#0f7c56")
        self.canvas.itemconfigure(self.status_dot, fill="#18c47f")
        self.status_label.configure(text=readiness.title)
        self.caption.deiconify()
        self._place_caption()
        self.source_label.configure(text=readiness.detail)
        self.translation_label.configure(text="正在等待网页音频")
    ...
    def stop(self) -> None:
        ...
        self.canvas.itemconfigure(self.bubble, fill="#0f78c9", outline="#075f9f")
    ...
    def _show_translation(self, result: BackgroundTranslation) -> None:
        if not self._active:
            return
        self.status_label.configure(text="实时中文字幕")
    ...
    def _create_trail_windows(self) -> list[Any]:
        ...
        for alpha in (0.24, 0.18, 0.13, 0.09, 0.06):
            ...
            if self._launcher_image is not None:
                canvas.create_image(32, 32, image=self._launcher_image)
            else:
                canvas.create_oval(7, 7, 57, 57, fill="#0f78c9", outline="#8bd4ff", width=2)
                canvas.create_text(
                    32,
                    31,
                    text="译",
    ...
    def _launcher_geometry(self) -> str:
        if self._launcher_position is None:
            x = self.root.winfo_screenwidth() - 104
            y = 220
        else:
    ...
    def _clamp_launcher_position(self, x: int, y: int) -> tuple[int, int]:
        max_x = max(0, self.root.winfo_screenwidth() - 64)
        max_y = max(0, self.root.winfo_screenheight() - 64)
        return min(max(0, x), max_x), min(max(0, y), max_y)
